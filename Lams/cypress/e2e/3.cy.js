describe('user_registration', () => {
    it('passes', () => {
        const fnameBang = ['রাসেল'];
        const lnameBang = ['শাহরিয়ার'];
        const fnameEng = ['Rashel'];
        const lnameEng = ['Shahriar'];
        ...
        */
        ...
        const banRandomFname = fnameBang[Math.floor(Math.random() * fnameBang.length)];
        cy.get('#\\:r0\\:').should('be.visible').type(fnameBang);
        // Select a random last name bangla from the array
        const banRandomLname = lnameBang[Math.floor(Math.random() * lnameBang.length)];
       ...
       // Total = int(element_list[0])
       // capchabox = cy.get(By.XPATH, "//input[@name='captchaResult']").send_keys(str(Total))
       ...
       cy.get('.MuiTypography-root[aria-readonly="true"]').invoke('text').then((captchaText) => {
        ...
        //cy.get('button.css-1kjn3vt[jf-ext-button-ct="create account"]').click();
